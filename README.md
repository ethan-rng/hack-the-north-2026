...
Ground coordinates are `(x,z)`; `y` is renderer height only. Generation supports 6–24 points of interest. Airport layouts have a terminal spine and gate concourse; malls and small venues use corridors; neighborhoods use street blocks; parks use a circulation loop. Places carry illustrative rectangular footprints, rotations, zones and entrance positions. Rendered building models fit those footprints, and the renderer and movement engine share corridor polylines. Generic venues retain the force layout. Inferred paths do not enforce real security boundaries or indoor floors.

Visual context records the scene setting, architectural character, vegetation and design notes. Buildings use a shared wall/roof/trim/glass palette; semantic fallback selection prioritizes actual function over unique silhouettes (repeated cafes remain cafes). Indoor retail renders as concourse-facing storefronts. Setting-specific scenery includes terminal aprons, tiled interiors and planters, streets and background blocks, coastal water, and park or rural vegetation. Background scenery is decorative and adds no simulated destinations. These visual interpretations are illustrative, not surveyed architectural replicas.

Custom building requests receive the venue, scenario, place function, zone, footprint and available supporting excerpts. Context and palette are included in cache keys, and generated primitive materials distinguish masonry, glass, metal, wood and roofs. Missing or invalid custom geometry uses a compatible library or generic asset fallback.

Named places can carry geographic coordinates with source IDs and a verbatim supporting excerpt. Compilation requires the place name and both coordinate values in that excerpt, checks it against actual retrieved records, rejects duplicate/out-of-area anchors, and projects accepted coordinates using a common scale. Unlocated places occupy a separate illustrative area; the source panel identifies which positions are sourced. Building outlines and paths remain illustrative even when positions are grounded. Travel speeds remain demo units rather than geographically calibrated walking speeds. Distances are omitted from Jev's choice context.
...
Setup resolves identity first (30-second search budget), then researches the roster, layout and operations in parallel (30 seconds each), followed by synthesis (35 seconds). The setup watchdog allows 105 seconds, with its alarm at 110 seconds. Source IDs, research topics, URLs, titles, retrieval timestamps, excerpts and supported field paths are stored with the frozen baseline. Field evidence must include a quote found in its cited retrieval record; unsupported fields remain assumed. Partial research retains successful topics. The source panel distinguishes user instructions, researched facts, inferred structure and assumed operational values. Failed retrieval is visibly assumption-based; failed synthesis uses a labeled generic fallback. A returned LLM summary without tool results is never counted as successful research.
...
Venue-specific populations use synthetic cohorts (families, shoppers, workers on breaks, local households and passenger groups) with shared purposes and destinations, correlated budgets and patience, and individual variation. Passenger groups share a gate and journey deadline. Most visitors open already distributed through the venue, with initial activities and no fabricated sales; later groups remain absent until their scheduled simulation time, then enter together. Group context and intended departure times are included in Jev decisions. Groups still make individual choices, and intended departures are not enforced exit events. Generic venues retain the legacy population behavior. Opening the scenario makes no Jev calls and leaves everyone paused. Each event is interpreted, then the backend records exactly **30 simulated seconds** before the browser plays the result and automatically pauses. Playback supports 0.25×, 0.5×, 1×, 2× and 4× speed, pause, and forward/backward scrubbing. Inspectors and metrics follow the selected frame; playback makes no inference requests.
